...
Проверяем стабильную работу: командой docker ps -a **(контейнеры в статусе Up, запущены успешно)** и командой curl -L http://127.0.0.1:8090 **возвращает Ошибку 503 в качестве ответа** время и локальный IP-адрес.  

6) Подключеамся к БД mysql с помощью команды docker exec -ti <имя_контейнера> mysql -uroot -p<пароль root-пользователя>. Вводим последовательно команды: show databases; use <имя вашей базы данных(по-умолчанию virtd, как это указано в .env)>; show tables; SELECT * from requests LIMIT 10;. 
